...
const plageHoraireJoiSchema = Joi.object({
    debut: Joi.string().pattern(/^([01]\d|2[0-3]):([0-5]\d)$/).required().messages({'string.pattern.base': 'L\'heure de début doit être au format HH:MM'}),
    fin: Joi.string().pattern(/^([01]\d|2[0-3]):([0-5]\d)$/).required().messages({'string.pattern.base': 'L\'heure de fin doit être au format HH:MM'})
});
...
exports.updateProfilTravailleurSchema = Joi.object({
    ...
    dateDeNaissance: Joi.date().iso().optional().messages({
        'date.format': 'Format de date de naissance invalide (YYYY-MM-DD attendu).',
    }),
    titreProfessionnel: Joi.string().trim().max(100).allow('').optional(),
    localisation: Joi.object({
        adresseTextuelle: Joi.string().trim().max(255).allow('').optional(),
        ville: Joi.string().trim().min(2).max(50).optional(),
        quartier: Joi.string().trim().max(50).allow('').optional(),
    }).optional(),
    presentation: Joi.string().max(1500).allow('').optional(),

    // Ajoute ici le bon schéma pour le tableau d'expériences :
    experience: Joi.array().items(
        Joi.object({
            domaine: Joi.string().trim().min(2).max(100).required(),
            annees: Joi.number().min(0).max(60).required()
        })
    ).optional(),

    disponibilite: Joi.object({
        lundi: Joi.array().items(plageHoraireJoiSchema).optional(),
        mardi: Joi.array().items(plageHoraireJoiSchema).optional(),
        mercredi: Joi.array().items(plageHoraireJoiSchema).optional(),
        jeudi: Joi.array().items(plageHoraireJoiSchema).optional(),
        vendredi: Joi.array().items(plageHoraireJoiSchema).optional(),
        samedi: Joi.array().items(plageHoraireJoiSchema).optional(),
        dimanche: Joi.array().items(plageHoraireJoiSchema).optional(),
...
exports.createAnnonceSchema = Joi.object({
    ...
    avantages: Joi.array().items(Joi.string().trim()).optional(),
    localisation: Joi.object({
...
exports.updateAnnonceSchema = Joi.object({ // Similaire à create, mais tous les champs sont optionnels
    ...
    categorieId: Joi.string().hex().length(24).optional().messages({ // Valide un ObjectId MongoDB
        'string.hex': 'ID de catégorie invalide.',
        'string.length': 'ID de catégorie invalide.',
    }),
    localisation: Joi.object({
    ...
    estUrgent: Joi.boolean().optional(),
    dureeMission: Joi.object({
    valeur: Joi.number().integer().min(1).optional(),
    unite: Joi.string().valid('jours', 'semaines', 'mois', 'annees').optional(),
    }).when('dateDebutSouhaitee', { // Rendre dureeMission requis si dateDebutSouhaitee est fournie
        is: Joi.exist(),
        then: Joi.optional(),
        otherwise: Joi.optional()
...
exports.updateStatutCandidatureSchema = Joi.object({
    statut: Joi.string().valid('vue', 'terminee_manuellement', 'preselectionnee', 'rejete', 'acceptee').required().messages({
